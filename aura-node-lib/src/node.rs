...
struct ProposalBuffer {
    entries: HashMap<StreamId, ProposalEntry>,
}
        ...
        tokio::select! {
            Some(msg) = channels.consensus.recv() => {
                ...
                match msg {
                    ...
                    AppMsg::RestreamProposal { height, round, reply } => {
                        ...
                        if reply.send(()).is_err() { error!("AppLoop: Failed to send RestreamProposal reply"); }
                    }
