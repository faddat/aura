...
impl ProposalBuffer {
    ...
    fn insert(&mut self, part: StreamMessage<ProposalPart>) {
        let id = part.stream_id.to_bytes().to_vec();
        match &part.content {
    ...
    async fn restream(&self, channels: &mut Channels<TestContext>, id: &StreamId) -> eyre::Result<()> {
        if let Some(entry) = self.entries.get(&id.to_bytes().to_vec()) {
    ...
    fn remove(&mut self, id: &StreamId) {
        self.entries.remove(&id.to_bytes().to_vec());
    }
}
                    ...
                    AppMsg::ReceivedProposalPart { from, part, reply } => {
                        ...
                        match &part.content {
                            ...
                            StreamContent::Fin | StreamContent::Data(ProposalPart::Fin(_)) => {
                                if let Some(entry) = proposal_buffer.entries.get(&stream_key.to_bytes().to_vec()) {
